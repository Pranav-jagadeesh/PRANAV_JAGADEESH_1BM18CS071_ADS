...
void TwoThreeNode::remove(int k)
{
    ...
    else
    {
        if (leaf)
        {
            cout << "\nThe key " << k << "is nonexistant in the tree.";
            return;
        }
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        cout << "\nEnter the "<< i + 1 <<" node to be inserted : ";
        int node;
        cin >> node;
        t.insert(node);
    ...
    cout << "\nEnter number of nodes to be Deleted from tree : ";
    cin >> n;
    for (int i = 0; i < n; i++)
    {
        cout << "\nEnter the node to be deleted from tree : ";
        int node;
        cin >> node;
        t.remove(node);
        cout << "\nThe tree structure after deletion of " << node << " is : ";
        t.traverse();
        cout << endl;
    }
